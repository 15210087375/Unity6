...
namespace Lean.Pool
{
	...
	public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public static bool TryFindPoolByCloneDeSpawned(GameObject clone, ref LeanGameObjectPool pool)
		{
			foreach (var instance in Instances)
			{
				// Search hash set
				...
				// Search list
				for (var j = instance.despawnedClones.Count - 1; j >= 0; j--)
				{
					if (instance.despawnedClones[j] == clone)
					{
						pool = instance;
		...
		public bool TrySpawn(ref GameObject clone, Vector3 localPosition, Vector3 localRotation, Vector3 localScale,
			Transform parent, bool worldPositionStays)
		{
			return TrySpawn(ref clone, localPosition, Quaternion.Euler(localRotation), localScale,parent,worldPositionStays);
		}
		/// <summary>This will either spawn a previously despawned/preloaded clone, recycle one, create a new one, or return null.</summary>
