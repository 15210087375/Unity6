    ...
    public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        */
        ...
        void UpdateContentSize()
        {
            ...
            //Debug.LogError($"s={size} , listCount={ItemList.Count}");
            if (mIsVertList)
            {
        ...
        #region custom
        // content size 是慢慢扩展的 verticalNormalizedPosition的值会不准确
        public void MoveTo(float rate, float timer)
        {
            var cur = mScrollRect.verticalNormalizedPosition;
            DG.Tweening.DOTween.To(()=>cur,  (x) =>
            {
                mScrollRect.verticalNormalizedPosition = x;
            },rate,timer);
        }
