...
namespace ExtendUI
{
    ...
    {
        [SerializeField]
        private bool slicedClipMode = false;
        [SerializeField]
        private bool shapeMode = false;
        [SerializeField,ShowIf("m_shapeMode")]
        private float shapeOffset;
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            switch (type)
            {
                case Type.Filled when slicedClipMode && (fillMethod == FillMethod.Horizontal || fillMethod == FillMethod.Vertical) && hasBorder:
            ...
            if (shapeMode)
            {
                ...
                vertex.position += Vector3.right * shapeOffset;
